...
class Task:
    ...
    #     max_x=0
    #     for tg_bx in tgs_bx:
    #         intersect = self.intersection(bone_bx, tg_bx)
    #         intersect_percents.append(self.area(intersect)/self.area(tg_bx))
    #         if tg_bx[3] <= min_y:
    #             min_y = tg_bx[3]
    #         if tg_bx[2] > max_x:
    #             max_x = tg_bx[2]
    #     max_percent=max(intersect_percents)
    #     min_percent=min(intersect_percents)
    #     print '_check_secure: max intersect: {}, min intersect: {}'.format(max_percent, min_percent)
    #     print '_check_secure: min_y: {}, max_x: {} bone_bx: {}'.format(min_y, max_x, bone_bx)
    #     succ=(max_percent>=1.0) and (min_percent>=0.8)
    #     if succ:
    #         if (min_y - bone_bx[1]) > 0.5 * (bone_bx[3] - bone_bx[1]):
    #             return 'wrong position'
    #         elif (max_x - bone_bx[0]) > 0.3 * (bone_bx[2] - bone_bx[0]):
    #             return 'wrong position'
    #     return succ
        ...
        if self.current_state == "nothing":
            ...
            if header != None and ('gaugeColor' in header):
                if header['gaugeColor'] == 'green':
                    ...
                    # should be green
                    result['speech'] = "The color is wrong. Please measure the bone thickness of the fracture"
                    image_path = "images_feedback/measure.png"
                    result['image'] = cv2.imread(image_path) if image_path else None
                    header['clear_color']=True
                    self.current_state = "measure"
            ...
            print 'sa_bxes:{}\nsaf_bxes:{}'.format(sa_bxes, saf_bxes)
            if object_counts['sbv'] > 0 and len(saf_bxes) > 0 and saf_bxes[0][-2] > 0.6 and (object_counts['sa']==0 or (len(sa_bxes)>0 and sa_bxes[0][-2] < 0.6)):
                self.cnt=0
                ...
                if self._check_secure(objects) == True:
                    if self.cnt < 2:
                        ...
                        self.cnt=0
                        result['speech'] = "Put the green drill driver onto the power drill"
            ...
            if object_counts['bn'] > 0 or object_counts['fn'] > 0:
