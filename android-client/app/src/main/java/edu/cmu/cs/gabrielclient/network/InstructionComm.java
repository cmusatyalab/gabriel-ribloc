...
import edu.cmu.cs.gabriel.instruction.Protos.Ribloc;
        ...
        this.consumer = new Consumer<Protos.ResultWrapper>() {
            ...
            public void accept(Protos.ResultWrapper resultWrapper) {
                try {
                    ...
                    // transmit or clear speech recognition result
                    if (engineFields.getRibloc().getClearColor()){
                        GabrielClientActivity.gaugeColor = null;
                        // unset clearColor
                        EngineFields.Builder infoFieldsBuilder = EngineFields.newBuilder();
                        infoFieldsBuilder.setUpdateCount(engineFields.getUpdateCount());
                        Ribloc.Builder infoRiblocBuilder = Ribloc.newBuilder();
                        infoRiblocBuilder.setClearColor(false);
                        infoFieldsBuilder.setRibloc(infoRiblocBuilder);
                        engineFields = infoFieldsBuilder.build();
                    }
                    if (GabrielClientActivity.gaugeColor!=null){
                        EngineFields.Builder infoFieldsBuilder = EngineFields.newBuilder();
                        infoFieldsBuilder.setUpdateCount(engineFields.getUpdateCount());
                        Ribloc.Builder infoRiblocBuilder = Ribloc.newBuilder();
                        infoRiblocBuilder.setGaugeColor(GabrielClientActivity.gaugeColor);
                        infoFieldsBuilder.setRibloc(infoRiblocBuilder);
                        engineFields = infoFieldsBuilder.build();
                    }
